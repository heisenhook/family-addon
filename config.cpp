...
std::vector<std::string> ConfigManager::GetConfigs( ) {
	...
	if ( created_cfg ) {
		...
		// create cheat folder 
		std::wstring cheat_folder = full_path.wstring( ) + XorStr( L"\\heisenhook" );
	...
	HANDLE preset_file = FindFirstFileA( "heisenhook\\configs\\*.nap", &find_data );
	...
	if ( preset_file != INVALID_HANDLE_VALUE ) {
		do {
			...
			// no.
			if ( s == "csgodatarec.nap" )
				continue;

			if ( s == "default.nap" )
				continue;

			size_t pos = s.find( ".nap" );
			if ( pos == std::string::npos ) // not found
				continue;

			// erase exactly 4 chars, not 5 (".nap" is 4)
			s.erase( pos, 4 );
...
void ConfigManager::LoadConfig( std::string configname, bool load_user_data ) {
	std::ifstream input_file = std::ifstream( ( XorStr( "heisenhook\\configs\\" ) + configname + XorStr( ".nap" ) ).c_str( ) );
	if ( load_user_data ) {
...
void ConfigManager::SaveConfig( std::string configname, bool load_user_data ) {
	if ( load_user_data ) {
		...
		std::ofstream o( ( XorStr( "heisenhook\\configs\\" ) + configname + XorStr( ".nap" ) ).c_str( ) );
		if ( !o.is_open( ) )
			return;
...
void ConfigManager::RemoveConfig( std::string configname ) {
	std::remove( ( XorStr( "heisenhook\\configs\\" ) + configname + XorStr( ".nap" ) ).c_str( ) );
}
...
void ConfigManager::CreateConfig( std::string configname, bool hidden ) {
	std::ifstream input_file = hidden ?
		...
		std::ifstream( ( XorStr( "heisenhook\\configs\\" ) + configname + XorStr( ".nap" ) ).c_str( ) );
...
void ConfigManager::OpenConfigFolder( ) {
	...
	std::wstring str = full_path.wstring( ) + XorStr( L"\\heisenhook\\configs" );
